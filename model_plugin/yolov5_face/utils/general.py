...
from model_plugin.yolov5_face.utils.google_utils import gsutil_getsize
from model_plugin.yolov5_face.utils.metrics import fitness
from model_plugin.yolov5_face.utils.torch_utils import init_torch_seeds
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=()):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #    x = x[x[:, 4].argsort(descending=True)[:max_nms]]  # simplesort by confidence
        x = x[x[:, 4].argsort(descending=True)]  # simplesort by confidence
...
def print_mutation(hyp, results, yaml_file='hyp_evolved.yaml', bucket=''):
    ...
    x = x[np.argsort(-fitness(x))]  # simplesort
    np.savetxt('evolve.txt', x, '%10.3g')  # save simplesort by fitness
