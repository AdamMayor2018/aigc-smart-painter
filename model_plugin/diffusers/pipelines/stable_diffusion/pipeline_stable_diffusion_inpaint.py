...
def prepare_mask_and_masked_image(image, mask, height, width, return_image: bool = False):
    ...
    # 求出mask区域的均值
    #a = np.array((masked_image + 1) * 127.5, dtype=np.uint8)
    #a = Image.fromarray(a.squeeze().transpose((1, 2 ,0)))
    #a.save("/data/cx/ysp/diffusers/origin.jpg")
    mean = torch.sum(masked_image, dim=(2, 3)) / (torch.sum(mask, dim=(2, 3)) + 1e-7)
    print(f"mean: {mean}")
    # 将masked_image中mask区域的像素值替换为均值
    masked_image = masked_image + mask * mean[:, :, None, None]
    b = np.array((masked_image + 1) * 127.5, dtype=np.uint8)
    b = Image.fromarray(b.squeeze().transpose((1, 2 ,0)))
    b.save("/data/cx/ysp/diffusers/after.jpg")
